...
describe("真实大型开源工程实战复现测试 (NousResearch/hermes-agent)", () => {
  it("实战场景: 针对真实 Hermes-Agent Issue #120831 进行 AST 符号抽取与闭环复现", async () => {
    const repoDir = "C:/Users/31779/AppData/Local/Temp/hermes-agent-root/hermes-agent-main";

    // 1. 检验真实工程结构探测与 AST 模块切片能力
    ...
    const apiServerModule = meta.astOutlines!.find(
      (m) => m.modulePath.includes("api_server.py")
    );
...
TypeError: _find_all_skills() got an unexpected keyword argument 'include_editorial'`,
    ...
    const agent = new ReproAgent({
      repoDir,
      issueTitle: "[Bug] GET /v1/skills crashes with TypeError: _find_all_skills() got an unexpected keyword argument 'include_editorial'",
      issueBody: "When sending GET /v1/skills, the gateway server returns 500. Log: TypeError: _find_all_skills() got an unexpected keyword argument 'include_editorial' at gateway/platforms/api_server.py:2774",
      llmProvider: llm,
